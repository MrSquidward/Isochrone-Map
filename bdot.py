...
for row in arcpy.da.SearchCursor(roads_fc, ["SHAPE@", "SHAPE@LENGTH"]):
    f_node = model.Node(int(row[0].firstPoint.X), int(row[0].firstPoint.Y))
    l_node = model.Node(int(row[0].lastPoint.X), int(row[0].lastPoint.Y))
    e_length = row[1]
    e = model.Edge(f_node.id, l_node.id, e_length)
    edges.append(e)
    d[f_node.id] = f_node
    d[l_node.id] = l_node
...
torun_skjz = model.Graph(edges, nodes)
